...
connect(MONGO_URL)
    ...
    api.get("/users/search", (req, res, next) => {
        try {
            ...
            jwt
                ...
                .then((payload) => {
                const { sub: userId } = payload;
                const { query, sortField, sortOrder, pageNumber, pageSize } = req.query;
                return logic
                    .findUsers(userId, query, sortField, sortOrder, parseInt(pageNumber), parseInt(pageSize))
    ...
    api.use((error, req, res, next) => {
        let status = 500, errorName = SystemError.name, message = error.message;
