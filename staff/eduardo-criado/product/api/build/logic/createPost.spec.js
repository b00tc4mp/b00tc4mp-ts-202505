...
describe.only("createPost", () => {
    ...
    it("creates a new post from a valid user", () => {
        return registerUser("John Doe", "johndoe@example.com", "johndoe", "password", "http://image.com/avatar")
            .then((authorId) => {
            return createPost(authorId, "Post Title", "Post Description", "http://image.com/post").then(() => {
                return authorId;
            });
        })
            .then((authorId) => {
            return PostRepository.findAll().then((posts) => {
                ...
                expect(posts[0].author).to.equal(authorId);
    ...
    it("fails on invalid author", () => {
        let errorThrown;
        return createPost("", "Post Title", "Post Description", "http://image.com/post")
            .catch((error) => {
            errorThrown = error;
        })
            .then(() => {
    ...
    it("fails on invalid description type", () => {
        ...
        try {
            /* @ts-ignore */
            createPost("123456789012345678901234", "Post Title", "", "http://image.com/post");
        }
        catch (error) {
            errorThrown = error;
        }
